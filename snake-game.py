...
#
# UPDATE: Used proper python syntax in comments.
...
count = 0
snake = [[15,15],[0,0]]                                                             # snake[0] is the starting position. Each new list in the list is a "body part" so in this case the snake starts with 2 body parts.
...
while key != 27:                                                                    # This is where the fun happens. 27 ASCII code for 'ESC'. When you hit it, while it's moving. It will break the while loop and end the window.
    ...
    if snake[0] in snake[1:]:                                       # If one of the snakehead's [x,y] values match another value within the snakes list. It will exit.  
        break
    ...
    if snake[0] == food:                                            # When snakes postion on the window matches the foods position = "eaten". This if statement will execute.
        ...
        while food == []:                                           # Executed because the window is hungry for food.
            ...
            if food in snake:                                       # If the new generated foods list values match a set of list values within the snakes list, it will reset the while. 
                food = []                                          
        win.addch(food[0], food[1], '$')                            # WHen the new food passes the while validator, it will then be assigned to actual food variable.
    else:    
        last = snake.pop()                                          # Since the snake.insert statement above is adding new snake list (heads), if the snake doesn't eat the food, then it's here that the snake.pop() pops the last [x,y] list in the snake list (the tail end of the simulated snake)
